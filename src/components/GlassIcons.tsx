...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
    ...
    return (
        <div className={`mx-auto py-[30px] overflow-visible ${className || ''}`}>
            {items.map((item, index) => (
                <button
                    key={index}
                    type="button"
                    aria-label={item.label}
                    className={`relative bg-transparent outline-none w-[60px] h-[60px] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
                        item.customClass || ''
                    }`}
                >
          <span
